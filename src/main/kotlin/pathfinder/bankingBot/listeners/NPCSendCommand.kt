...
) : SlashCommandInterface(eventWaiter, "npc_send", "Send money to an NPC account.") {
                    ...
                    try {
                        ...
                        when {
                            ...
                            gold > sender.balance -> it.editOriginal("You cannot send more gold than you have.")
                                ...
                                val taxAccount = recipient.accountType.taxConfig?.targetAccount
                                sender.send(gold, recipient)
                                recipient.receive(gold, sender)
                                characterRepository.saveAllAndFlush(listOfNotNull(sender.character, recipient.character, taxAccount?.character))
